...
- Required dependency: `local_outcomemap` `2026081700` or later (security-hardened 0.9.1 service boundary)
...
- Per-question editor bound to the displayed exact question version, with context-scoped outcome revalidation and POST-only draft mutations
...
- Explicit read, mapping, and Moodle question-capability enforcement on direct routes and every posted question, failing closed when `local_outcomemap` is absent
- Escaped form options and notifications, bounded keyword filters, and strictly validated bulk confirmation payloads
